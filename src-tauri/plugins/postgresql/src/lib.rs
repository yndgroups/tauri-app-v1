...
//插件状态，配置字段
pub struct PostgreSqlPlugin<R: Runtime> {
...
#[tauri::command]
fn initialize() {
    println!("postgresql -  initialize")
}

// invoke('plugin:awesome|do_something')
#[tauri::command]
fn do_something() {
    println!("postgresql -  do_something")
}

impl<R: Runtime> PostgreSqlPlugin<R> {
    pub fn new() -> Self {
        Self {
...
impl<R: Runtime> Plugin<R> for PostgreSqlPlugin<R> {
