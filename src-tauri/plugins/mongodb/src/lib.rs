...
//插件状态，配置字段
pub struct MongodbPlugin<R: Runtime> {
...
#[tauri::command]
fn initialize() {
    println!("mongodb -  initialize")
}

// invoke('plugin:awesome|do_something')
#[tauri::command]
fn do_something() {
    println!("mongodb -  do_something")
}

impl<R: Runtime> MongodbPlugin<R> {
    pub fn new() -> Self {
        Self {
...
impl<R: Runtime> Plugin<R> for MongodbPlugin<R> {
