...
//插件状态，配置字段
pub struct OraclePlugin<R: Runtime> {
...
#[tauri::command]
fn initialize() {
    println!("oracle -  initialize")
}

// invoke('plugin:awesome|do_something')
#[tauri::command]
fn do_something() {
    println!("oracle -  do_something")
}

impl<R: Runtime> OraclePlugin<R> {
    pub fn new() -> Self {
        Self {
...
impl<R: Runtime> Plugin<R> for OraclePlugin<R> {
