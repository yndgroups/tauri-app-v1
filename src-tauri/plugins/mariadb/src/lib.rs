...
//插件状态，配置字段
pub struct MariadbPlugin<R: Runtime> {
...
#[tauri::command]
fn initialize() {
    println!("mariadb -  initialize")
}

// invoke('plugin:awesome|do_something')
#[tauri::command]
fn do_something() {
    println!("mariadb -  do_something")
}

impl<R: Runtime> MariadbPlugin<R> {
    pub fn new() -> Self {
        Self {
...
impl<R: Runtime> Plugin<R> for MariadbPlugin<R> {
