...
//插件状态，配置字段
pub struct KingbasePlugin<R: Runtime> {
...
#[tauri::command]
fn initialize() {
    println!("kingbase -  initialize")
}

// invoke('plugin:awesome|do_something')
#[tauri::command]
fn do_something() {
    println!("kingbase -  do_something")
}

impl<R: Runtime> KingbasePlugin<R> {
    pub fn new() -> Self {
        Self {
...
impl<R: Runtime> Plugin<R> for KingbasePlugin<R> {
