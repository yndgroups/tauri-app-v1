...
//插件状态，配置字段
pub struct RedisPlugin<R: Runtime> {
...
#[tauri::command]
fn initialize() {
    println!("redis -  initialize")
}

// invoke('plugin:awesome|do_something')
#[tauri::command]
fn do_something() {
    println!("redis -  do_something")
}

impl<R: Runtime> RedisPlugin<R> {
    pub fn new() -> Self {
        Self {
...
impl<R: Runtime> Plugin<R> for RedisPlugin<R> {
