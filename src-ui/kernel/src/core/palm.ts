...
export default class Palm implements PalmEditor {
  ...
  loadPlugins() {
    return new Promise((resolve, reject) => {
      try {
        let ps = readDir('.palm/plugins', { dir: BaseDirectory.Home, recursive: true });
        ps.then((data: any) => {
          let pl = data.map(
            (item: any) =>
              new Promise(async (rs, rj) => {
                let file = await readTextFile(item.path + '/main.js');
                if (file && typeof file === 'string') {
                  // 读取配置
                  let config = await readTextFile(item.path + '/package.json');
                  // 读取样式
                  let style = await readTextFile(item.path + '/style.css');
